...
# esses imports dao um warning sobre tqdm
from timm.layers import DropPath
from timm.layers import to_2tuple # esse da pra remover
from timm.layers import trunc_normal_

# %% auto 0
__all__ = ['ConvBNPReLU', 'SACBlock', 'MRCIAMSingleChannel', 'MRCIAM', 'BasicEncoderBlock', 'CAM', 'EncoderModule', 'MLP',
...
class MRCIAMSingleChannel(nn.Module):
    ...
    """
    def __init__(self, 
                 parameters:dict # dictonary with the input channels and output channels of each ConvBNPReLU of the Class
                ): #
...
class MRCIAM(nn.Module):
    ...
    def __init__(self,
        ...
        self.mrciam_depth = MRCIAMSingleChannel(parameters["p1"])
        self.mrciam_reflectance = MRCIAMSingleChannel(parameters["p1"])
        self.mrciam_xyz = MRCIAMSingleChannel(parameters["p2"])

        if(self.using_reflectance):
          self.conv_1x1 = ConvBNPReLU(parameters["p1"]["output"] * 2 + parameters["p2"]["output"], parameters["output_conv"], kernel_size=1)
        else:
          self.conv_1x1 = ConvBNPReLU(parameters["p1"]["output"] + parameters["p2"]["output"], parameters["output_conv"], kernel_size=1)

        # SCA Block
        self.sac_block = SACBlock(parameters["output_conv"])

    def forward(self, xyz_tensor, depth_tensor, reflectance_tensor):
      mrciam_xyz =  self.mrciam_xyz(xyz_tensor)
      mrciam_depth = self.mrciam_depth(depth_tensor)
      if(self.using_reflectance):
        mrciam_reflectance = self.mrciam_reflectance(reflectance_tensor)

      if(self.using_reflectance):
        concat_out = torch.cat((mrciam_xyz, mrciam_depth, mrciam_reflectance), 1)
      else:
        concat_out = torch.cat((mrciam_xyz, mrciam_depth), 1)
...
class BasicEncoderBlock(nn.Module):
    ...
    """
    def __init__(self, 
                 parameters:dict # dictonary with the input channels and output channels of the encoder convolutions
                ):  
        super(BasicEncoderBlock, self).__init__()
        self.conv1 = ConvBNPReLU(parameters["in_channels"], parameters["conv2_in_channels"], kernel_size=3, padding=1)
        self.conv2 = ConvBNPReLU(parameters["conv2_in_channels"], parameters["conv2_out_channels"], kernel_size=3, padding=1)
        self.dilated_conv = ConvBNPReLU(parameters["conv2_out_channels"], parameters["dilated_conv_out_channels"], kernel_size=3, padding=2, dilation=2)

        residual_conv1_input_dimension = parameters["conv2_in_channels"] + parameters["conv2_out_channels"] + parameters["dilated_conv_out_channels"] + parameters["dilated_conv_out_channels"]
        self.residual_conv1 = ConvBNPReLU(residual_conv1_input_dimension, parameters["residual_out_channels"], kernel_size=1)
        self.residual_conv2 = ConvBNPReLU(parameters["in_channels"], parameters["residual_out_channels"], kernel_size=1)
...
class EncoderModule(nn.Module):
    ...
    """
    def __init__(self, 
                 parameters:dict # dictonary with the input channels and output channels of the encoder module convolutions
                ): # output size of basic encoder block
        super(EncoderModule, self).__init__()
        self.encoder_block = BasicEncoderBlock(parameters)
        # in_channels of CAM is the out_shape of BasicEncoderBlock, see BasicEncoderBlock
        self.cam = CAM(parameters["residual_out_channels"])
...
def window_partition(x:torch.Tensor, # (B, H, W, C)
                     window_size:tuple # window size (h, w)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, 
                 ...
                 qkv_bias:bool=True,                 # If True, add a learnable bias to query, key, value.
                 qk_scale:float=None,                # Override default qk scale of **head_dim *g\* -0.5 if set**
    ...
    def forward(self, 
                x:torch.Tensor,           # Input feature, tensor size (B, H*gW, C).
